...
class RLTrainer:
    ...
    def load_training_data(self, data_path: str = "ml-training/data/preprocessed"):
        ...
        logger.info(
            f"Benign: {np.sum(y_train_binary == 0)}, Attacks: {np.sum(y_train_binary == 1)}"
        )
    ...
    def save_final_model(self):
        ...
        metrics = {
            ...
            "final_accuracy": (
                self.episode_accuracies[-1] if self.episode_accuracies else 0
            ),
            "avg_accuracy_last_100": (
                np.mean(self.episode_accuracies[-100:])
                if len(self.episode_accuracies) >= 100
                else 0
            ),
            "final_f1": self.episode_f1_scores[-1] if self.episode_f1_scores else 0,
            "avg_f1_last_100": (
                np.mean(self.episode_f1_scores[-100:])
                if len(self.episode_f1_scores) >= 100
                else 0
            ),
    ...
    def plot_training_results(self):
        ...
        axes[0, 0].plot(
            self._smooth(self.episode_rewards, 50),
            linewidth=2,
        ...
        axes[0, 1].plot(
            self._smooth(self.episode_accuracies, 50),
            linewidth=2,
            label="Moving Average (50)",
        )
        ...
        axes[0, 2].plot(
            self._smooth(self.episode_f1_scores, 50),
            linewidth=2,
            label="Moving Average (50)",
        )
        ...
        axes[1, 0].plot(
            self._smooth(self.episode_precisions, 50),
            linewidth=2,
            label="Moving Average (50)",
        )
        ...
        axes[1, 1].plot(
            self._smooth(self.episode_recalls, 50),
            linewidth=2,
            label="Moving Average (50)",
        )
        ...
        axes[1, 2].plot(
            self._smooth(self.episode_losses, 50),
            linewidth=2,
            label="Moving Average (50)",
        )
    ...
    def evaluate(self, n_episodes: int = 100):
        ...
        logger.info(
            f"Average Accuracy: {np.mean(eval_accuracies):.3f} ± {np.std(eval_accuracies):.3f}"
        )
        logger.info(
            f"Average F1 Score: {np.mean(eval_f1_scores):.3f} ± {np.std(eval_f1_scores):.3f}"
        )
        logger.info("=" * 70 + "\n")
...
def main():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default="ml-training/data/preprocessed",
        help="Path to preprocessed data",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="rl-module/trained_models",
