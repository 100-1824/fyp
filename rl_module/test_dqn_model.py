...
class DQNModelTester:
    ...
    def __init__(self, model_path, test_data_path=None):
        ...
        self.test_data_path = Path(test_data_path) if test_data_path else Path(__file__).parent.parent
        self.model = None
        self.X_test = None
        self.y_test = None
    ...
    def load_test_data(self):
        ...
        for path in possible_paths:
            if path.exists():
                ...
                print(f"  Benign: {np.sum(self.y_test == 0)}, Attacks: {np.sum(self.y_test == 1)}")
                return True
    ...
    def test_sample_predictions(self, num_samples=10):
        ...
        for idx in indices:
            ...
            print(f"\nSample {idx}:")
            print(f"  True: {true_name:7s} | Predicted: {pred_name:7s} | Action: {action_names[action[0]]:10s} {match}")
            print(f"  Q-values: Allow={q_vals[0][0]:.3f}, Block={q_vals[0][1]:.3f}, Quarantine={q_vals[0][2]:.3f}")
...
def main():
    ...
    if not tester.load_test_data():
        ...
        print("Please provide test data using --data argument or place X_test.npy and y_test.npy in project root")
        return
