...
class IDSModelTrainer:
    ...
    def build_model(self) -> keras.Model:
        ...
        for i, layer_config in enumerate(self.config["model"]["layers"]):
            ...
            if "dropout" in layer_config and layer_config["dropout"] > 0:
                model.add(
                    layers.Dropout(layer_config["dropout"], name=f"dropout_{i+1}")
                )
        ...
        model.compile(
            optimizer=optimizer, loss=self.config["model"]["loss"], metrics=["accuracy"]
        )
    ...
    def get_callbacks(self) -> list:
        ...
        if self.config["training"]["early_stopping"]["enabled"]:
            early_stop = callbacks.EarlyStopping(
                ...
                restore_best_weights=self.config["training"]["early_stopping"][
        ...
        if self.config["training"]["checkpoint"]["enabled"]:
            checkpoint_path = (
                self.model_output_path / "checkpoints" / "model_checkpoint.keras"
            )
    ...
    def plot_training_history(self):
        ...
        output_path = (
            ...
            / f"{self.config['output']['model_name']}_training_history.png"
        )
    ...
    def plot_confusion_matrix(self, cm: np.ndarray):
        ...
        output_path = (
            self.model_output_path
            / f"{self.config['output']['model_name']}_confusion_matrix.png"
        )
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
