...
class PacketPreprocessor:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized PacketPreprocessor with {self.feature_count} features"
        )
                ...
                packet_info.update(
                    {
                        ...
                        "payload_size": (
                            len(packet[IP].payload) if packet[IP].payload else 0
                        ),
                        "flags": {
                            "fin": 0,
                            "syn": 0,
    ...
    def normalize_features(
        self, features: np.ndarray, method: str = "minmax"
    ) -> np.ndarray:
        """
    ...
    def preprocess_packet(
        self, packet, flow_tracker: "EnhancedFlowTracker"
...
class EnhancedFlowData:
    ...
    def compute_all_features(self) -> Dict[str, float]:
        ...
        if len(self.fwd_packets) >= 2:
            ...
            fwd_iats = [
                fwd_times[i + 1] - fwd_times[i] for i in range(len(fwd_times) - 1)
            ]
        ...
        if len(self.bwd_packets) >= 2:
            ...
            bwd_iats = [
                bwd_times[i + 1] - bwd_times[i] for i in range(len(bwd_times) - 1)
            ]
        ...
        # Header lengths
        fwd_header_total = (
            sum(p[4] for p in self.fwd_packets) if self.fwd_packets else 0
        )
        bwd_header_total = (
            sum(p[4] for p in self.bwd_packets) if self.bwd_packets else 0
        )
        features["Fwd Header Length"] = float(fwd_header_total)
        features["Fwd Header Length.1"] = float(
            fwd_header_total
        ...
        features["Avg Fwd Segment Size"] = (
            (fwd_bytes / total_fwd_packets) if total_fwd_packets > 0 else 0.0
        )
        features["Avg Bwd Segment Size"] = (
            (bwd_bytes / total_bwd_packets) if total_bwd_packets > 0 else 0.0
        )

        # Bulk features (simplified - need more complex logic for true bulk detection)
        features["Fwd Avg Bytes/Bulk"] = self.fwd_bulk_bytes / max(
            self.fwd_bulk_packets, 1
        )
        features["Fwd Avg Packets/Bulk"] = float(self.fwd_bulk_packets)
        features["Fwd Avg Bulk Rate"] = (
            (self.fwd_bulk_bytes / duration) if duration > 0 else 0.0
        )
        features["Bwd Avg Bytes/Bulk"] = self.bwd_bulk_bytes / max(
            self.bwd_bulk_packets, 1
        )
        features["Bwd Avg Packets/Bulk"] = float(self.bwd_bulk_packets)
        features["Bwd Avg Bulk Rate"] = (
            (self.bwd_bulk_bytes / duration) if duration > 0 else 0.0
        )
        ...
        features["min_seg_size_forward"] = (
            float(min(fwd_payloads)) if fwd_payloads else 0.0
        )
...
class EnhancedFlowTracker:
    ...
    def __init__(self, flow_timeout: int = 120, max_flows: int = 10000):
        ...
        logger.info(
            f"Initialized EnhancedFlowTracker (timeout={flow_timeout}s, max_flows={max_flows})"
        )
