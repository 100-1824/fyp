...
class RuleParser:
    ...
    VALID_PROTOCOLS = {'tcp', 'udp', 'icmp', 'ip', 'http', 'ftp', 'tls', 'smb', 'dns', 'ssh'}
    ...
    def parse_rule(self, rule_string: str, rule_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        self.rule_stats['by_action'][action] = self.rule_stats['by_action'].get(action, 0) + 1
        self.rule_stats['by_protocol'][protocol] = self.rule_stats['by_protocol'].get(protocol, 0) + 1
    ...
    def validate_rule(self, rule: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        # Validate severity
        if 'severity' in rule and rule['severity'] not in ['critical', 'high', 'medium', 'low']:
...
class RuleManager:
    ...
    def get_active_rules(self, protocol: Optional[str] = None,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats['active_rules'] = len([r for r in self.active_rules if r.get('enabled', True)])
        stats['total_loaded'] = len(self.active_rules)
        stats['disabled_rules'] = len([r for r in self.active_rules if not r.get('enabled', True)])

        # Calculate total hits
        stats['total_hits'] = sum(r.get('hit_count', 0) for r in self.active_rules)

        # Top rules by hits
        top_rules = sorted(self.active_rules, key=lambda r: r.get('hit_count', 0), reverse=True)[:10]
        stats['top_rules'] = [
            {'sid': r['sid'], 'msg': r.get('msg', ''), 'hits': r.get('hit_count', 0)}
            for r in top_rules if r.get('hit_count', 0) > 0
        ]
