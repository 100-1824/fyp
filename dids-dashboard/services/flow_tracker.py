...
class FlowData:
    ...
    def add_packet(
        self, size: int, timestamp: float, direction: str, flags: Dict[str, int]
    ):
        """Add packet to flow"""
    ...
    def extract_features(self) -> Dict[str, float]:
        ...
        # Flow duration
        duration = max(
            self.last_seen - self.start_time, 0.000001
        ...
        total_bytes = sum(p[0] for p in self.fwd_packets) + sum(
            p[0] for p in self.bwd_packets
        )
        ...
        all_timestamps = [p[1] for p in self.fwd_packets] + [
            p[1] for p in self.bwd_packets
        ]
        all_timestamps.sort()
        ...
        if len(self.fwd_packets) >= 2:
            ...
            fwd_iats = [
                fwd_timestamps[i + 1] - fwd_timestamps[i]
                for i in range(len(fwd_timestamps) - 1)
            ]
        ...
        if len(self.bwd_packets) >= 2:
            ...
            bwd_iats = [
                bwd_timestamps[i + 1] - bwd_timestamps[i]
                for i in range(len(bwd_timestamps) - 1)
            ]
        ...
        if total_packets > 0:
            all_sizes = [p[0] for p in self.fwd_packets] + [
                p[0] for p in self.bwd_packets
            ]
