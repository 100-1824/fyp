    ...
    try:
        ...
        if search:
            ...
            rules = [r for r in rules if
                    search_lower in r.get('msg', '').lower() or
                    search_lower in r.get('sid', '').lower() or
                    search_lower in r.get('classtype', '').lower()]
...
def get_top_rules():
    ...
    try:
        ...
        # Sort by hit count
        top_rules = sorted(all_rules, key=lambda r: r.get('hit_count', 0), reverse=True)[:limit]
