...
def call_service(
    service_name: str, endpoint: str, method: str = "GET", data: dict = None
):
...
def health():
    ...
    overall_status = (
        "healthy"
        if all(s["status"] == "healthy" for s in services_health.values())
        else "degraded"
    )
...
def analyze_packet():
    ...
    try:
        ...
        if app.config["ENABLE_RL_DETECTION"]:
            rl_data = {"packet": data, "ai_detection": ai_result if ai_result else {}}
        ...
        final_action = determine_final_action(
            results.get("detections", []), results.get("rl_decision", {})
        )
...
def determine_final_action(detections: list, rl_decision: dict) -> dict:
    ...
    for detection in detections:
        if (
            detection.get("severity") == "critical"
            or detection.get("action") == "block"
        ):
            return {
                "action": "block",
    ...
    if rl_decision.get("action") == "alert" or any(
        d.get("action") == "alert" for d in detections
    ):
        return {
            "action": "alert",
...
def get_recent_detections():
    ...
    result, error = call_service(
        "signature_detection", f"/detections/recent?limit={limit}"
    )
